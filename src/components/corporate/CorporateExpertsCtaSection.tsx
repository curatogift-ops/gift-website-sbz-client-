...
export default function CorporateExpertsCtaSection() {
  ...
  return (
    <section
      className="relative overflow-hidden bg-[#0c1428]"
    ...
    >
      ...
      <div className="section-container relative py-10 sm:py-12 lg:py-14">
        <div className="flex flex-col gap-10 lg:flex-row lg:items-center lg:gap-0">
          {/* Left — icon + headline */}
          <div className="flex gap-5 sm:gap-6 lg:min-w-0 lg:flex-[1.05] lg:pr-10">
            <div
            ...
            <div className="min-w-0 pt-1">
              <p className="font-sans text-[13px] font-normal leading-snug text-white/90 sm:text-sm">
                Looking for a reliable gifting partner for your business?
              </p>
              <h2
                id="corporate-experts-cta-heading"
                className="mt-2 font-sans text-[clamp(1.35rem,3.5vw,2rem)] font-semibold leading-[1.2] tracking-[-0.01em] text-white lg:mt-2.5 lg:text-[1.75rem] xl:text-[2rem]"
              >
                Let&apos;s create{' '}
          ...
          {/* Divider */}
          <div className="hidden w-px shrink-0 self-stretch bg-white/20 lg:block lg:mx-8" aria-hidden />

          {/* Right — features + CTA */}
          <div className="flex flex-col gap-8 lg:min-w-0 lg:flex-1 lg:flex-row lg:items-center lg:gap-10 xl:gap-12">
            <ul className="grid list-none grid-cols-1 gap-x-10 gap-y-4 sm:grid-cols-2 lg:gap-x-8 lg:gap-y-5">
              {CTA_FEATURES.map((item) => {
                const Icon = item.Icon;
                return (
                  <li key={item.label} className="flex items-center gap-3">
                    <Icon className="h-5 w-5 shrink-0 text-[#C9A96E]" strokeWidth={1.65} aria-hidden />
                    <span className="font-sans text-[13px] font-medium text-white sm:text-sm">{item.label}</span>
                  </li>
                );
              })}
            </ul>
            ...
            <div className="flex flex-col items-stretch sm:items-end lg:shrink-0">
              <a
                ...
                className="inline-flex min-h-[3rem] items-center justify-center gap-2.5 rounded-md bg-[#B8924F] px-6 py-3.5 font-sans text-[11px] font-bold uppercase tracking-[0.14em] text-white shadow-[0_8px_24px_-8px_rgba(0,0,0,0.45)] transition hover:bg-[#a67f42] sm:min-h-[3.25rem] sm:px-8 sm:text-xs sm:tracking-[0.16em]"
              >
                Contact our experts
                <ArrowRight className="h-4 w-4 shrink-0" strokeWidth={2.5} aria-hidden />
              </a>
              <p className="mt-3 text-right font-sans text-sm italic text-white/95 sm:text-[15px]">
                We&apos;re here to{' '}
